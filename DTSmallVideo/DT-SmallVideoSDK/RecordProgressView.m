...
-(void)setUp{
    ...
    line.frame = CGRectMake(self.frame.size.width *(_minTime/_maxTime), 0, 1.5, self.frame.size.height);
    line.backgroundColor = kBlack.CGColor;
    [self.layer  addSublayer:line];

    _currentProgressLayer = [CALayer layer];
    _currentProgressLayer.backgroundColor = [UIColor blackColor].CGColor;
...
-(void)cancelDeleteLastProgressView{
    if(_layerArray.count > 0){
        dispatch_async(dispatch_get_main_queue(), ^{
            [CATransaction begin];
            [CATransaction setDisableActions:YES];
            ((CALayer *)[_layerArray lastObject]).backgroundColor = kBlack.CGColor;
            [CATransaction commit];
        });
    }
}
...
-(CALayer *)generateNewProgressView{
    ...
    newProgreLayer.backgroundColor = kBlack.CGColor;
    CALayer *line = [CALayer layer];
    line.frame = CGRectMake(0, 0, 1.5, newProgreLayer.frame.size.height);
    line.backgroundColor = kBlack.CGColor;
